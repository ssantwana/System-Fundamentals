...
Test(sf_memsuite_student, free_block_shoulf_be_pre_when_prev_alloc_is_0, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	sf_block_info *info = NULL;
	void *x = sf_malloc(88);
	info = (x + 88);
	cr_assert(info->prev_allocated == 1 , "Prev allocated not correctly set 1 ");
	//cr_assert( ,"\nDiffrent block allocated than expected");
	void *y = sf_malloc(88);
	sf_free(x);
	cr_assert(info->prev_allocated == 0 , "Prev allocated not correctly set 2 ");
	void *z = sf_malloc(80);
	cr_assert(x == z ,"\nDiffrent block allocated than expected");
	cr_assert(info->prev_allocated == 1 , "Prev allocated not correctly set 3");
	//sf_malloc(sizeof(int));
	sf_free(y);
	sf_free(x);
...
Test(sf_memsuite_student, same_size_realloc_same_ptr, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(88);
	void *y = sf_realloc(x, 88);

	cr_assert_not_null(y, "y is NULL!");

	cr_assert(x == y, "Diffrent ptrs!");
...
Test(sf_memsuite_student, 0_size_realloc_NULL, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(88);
...
Test(sf_memsuite_student, ENOMEM_in_realloc, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(PAGE_SZ * 2 + 8 );
	void *y = sf_realloc(x, PAGE_SZ * 2 + 12);
	cr_assert(y == NULL, "y is not NULL!");

}


Test(sf_memsuite_student, regression_test, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *mem = sf_malloc(0);
	cr_assert(mem == NULL,"Does not work for 0 memory");
}
